...
namespace Zlipacket.Core.HSM
{
    ...
    public class StateMachine<TOwner>
    {
        ...
        public bool Fire(string trigger)
        {
            ...
            {
                if (triggerTransitions.TryGetValue((activeChain[i].GetType(), trigger), out var toType))
                {
                    ChangeStateInternal(toType);
                    return true;
                }
            }
            if (anyTriggerTransitions.TryGetValue(trigger, out var anyTarget))
            {
                ChangeStateInternal(anyTarget);
                return true;
            }
            return false;
        }
